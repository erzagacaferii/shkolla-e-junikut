<?php
    ...
    if(isset($_POST['login']))
    {
        ...
        $drejtimi = mysqli_real_escape_string($conn,$_POST['drejtimi']);
        ...
        else if (!preg_match("/^[a-zA-ZëË ]*$/",$drejtimi)) 
                {
                    array_push($errors, "vetem shkronjat lejohen");
                }
            
        $query = "SELECT * FROM nxenesi WHERE email='$email' LIMIT 1";
        ...
        //Kujdes!!! Kur e ruani passwordin ne SQL me gjatesi me te gjate.
         $password = md5($password);//encrypt i passwordit para se ta ruajme ne database
            ...
            //Kujdes te veqant tek insertimi i te dhenave!!!
                ...
                $regist ="INSERT INTO mesimdhenesi (emri,mbiemri,mosha,email,nrtelefonit,PASSWORD,confirmpassword,drejtimi )
                VALUES ('$emri','$mbiemri','$mosha','$email','$nrtelefonit','$password','$confirmpassword','$drejtimi')";
                
                $rows = "SELECT * FROM mesimdhenesi WHERE emri='$emri' AND mbiemri='$mbiemri'";
